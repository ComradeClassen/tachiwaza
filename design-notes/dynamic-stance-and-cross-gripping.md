...
- **Grip seating** (`grip_graph.attempt_engagement`, `match._seat_grips_for`): historically only ever seated the symmetric `LAPEL_HIGH` + `SLEEVE_HIGH` pair. HAJ-224 split `_seat_grips_for` to seat the lead hand first (lapel) with the off-hand sleeve following a beat later — no longer an atomic double-seat. HAJ-238 then added the `lead_plan` choice so that lead hand can be a `CROSS` or `PISTOL` instead of the orthodox lapel (see Phase 2 below). *Pre-HAJ-238 cross/pistol grips were never created by the engine.*

**So the gap was two concrete things:** stance never *changed*, and cross-grips were never *seated/selected*. The leverage math, the matchup derivation, and the regulation clock were already there. Both gaps are now closed (Phases 1–2).
...
- **Phase 2 — Cross-grip seating (Ring 1). ✅ IMPLEMENTED (HAJ-238).** MIRRORED cross-grip election seating `CROSS` and `PISTOL` edges (incl. PISTOL defensive frame + sode setup); cross-grip-as-disruptor + the experienced "route through it to my preferred grip" counter (§4.4); leans on existing `stance_parity` + `unconventional_clock` and the HAJ-224 lead-hand-first split. Kenka-yotsu narration family. *Landed as `src/cross_grip.py` (`select_lead_grip` / `select_defensive_frame` / `lead_grip_placement` + the kenka narration helpers) wired into the grip cascade via `match._elect_lead_grip` / `_emit_lead_grip_narration`, threaded through `_seat_grips_for(lead_plan=…)` and `_apply_engaged_response`. Election is MIRRORED-only, gated by the kenka-yotsu comfort axis and weighted by archetype / fight_iq; the disruptor relaxes the gate under grip pressure; the experienced counter fires on `(fight_iq × kenka comfort)`. PISTOL serves an offensive sode setup (`identity.style_dna["sode_setup_tendency"]`) and a defensive sode-tori frame in the cascade DEFENSIVE branch. The unconventional shido clock fires on un-attacked cross/pistol grips for free (already wired). `CROSS_GRIP` events carry mat-side `coach_prose`. Tests: `tests/test_cross_grip.py`. Note: in default ai-yotsu play these only appear after a Phase-1 stance switch reaches MIRRORED (rare for the orthodox-vs-orthodox default roster) — a kenka-yotsu hunter in the roster (via `stance_matchup_comfort` + `sode_setup_tendency`) would exercise them routinely; that's roster authoring, not engine work.*
